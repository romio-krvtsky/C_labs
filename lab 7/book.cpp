...
void load_book_to_file(Book* book, int fl, int count) // 1 - to books file, 2 - to order file
{
    ...
    if (fl == 1) snprintf(buf, radix, "%d", book->books_amount); //itoa(book->books_amount, buf, radix);
    else snprintf(buf, radix, "%d", count); //itoa(count, buf, radix);
    strcat(buf, "\n");
    fputs(buf, file);
    snprintf(buf, radix, "%d", book->cost_in);
    //itoa(book->cost_in, buf, radix);
    strcat(buf, "\n");
    fputs(buf, file);
    snprintf(buf, radix, "%d", book->cost_out);
    //itoa(book->cost_out, buf, radix);
    strcat(buf, "\n");
    fputs(buf, file);
    free(buf);
    fclose(file);
...
void books_file_rewrite(ListBook* list)
{
    ...
    while (temp)
    {
        ...
        snprintf(buf, radix, "%d", temp->books_amount);
        //itoa(temp->books_amount, buf, radix);
        strcat(buf, "\n");
        fputs(buf, file);
        snprintf(buf, radix, "%d", temp->cost_in);
        //itoa(temp->cost_in, buf, radix);
        strcat(buf, "\n");
        fputs(buf, file);
        snprintf(buf, radix, "%d", temp->cost_out);
        //itoa(temp->cost_out, buf, radix);
        strcat(buf, "\n");
        fputs(buf, file);
        free(buf);
        temp = temp->next;
